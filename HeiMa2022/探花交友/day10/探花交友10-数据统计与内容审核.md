...
<font color=red><b>`tanhua-admin`</b></font>工程中ManageController编写冻结方法
...
<font color=red><b>`tanhua-admin`</b></font>工程中ManageService编写冻结方法
...
<font color=red><b>`tanhua-admin`</b></font>工程中ManageController编写解冻方法
...
<font color=red><b>`tanhua-admin`</b></font>工程中ManageService编写解冻方法
...
修改<font color=red><b>`tanhua-admin`</b></font>工程中ManageService的根据id查询方法，设置冻结状态
...
在<font color=red><b>`tanhua-app-server`</b></font>项目中新建`UserFreezeService`，编写校验方法。判断用户冻结状态
...
<font color=red><b>`tanhua-app-server`</b></font>项目发送短信验证码的接口, 需要校验用户是否被冻结
...
> <font color=red><b>`tanhua-app-server`</b></font>模块中，发布动态和发布评论等接口也需要做相应的校验工作。
...
在<font color=red><b>`tanhua-app-server`</b></font>模块中配置发送消息的工具类
...
在<font color=red><b>`tanhua-admin`</b></font>模块中配置消息监听器，监听日志消息。
...
修改<font color=red><b>`tanhua-admin`</b></font>模块引导类，开启SpringTask功能支持
...
在<font color=red><b>`tanhua-admin`</b></font>模块配置定时任务类
...
在<font color=red><b>`tanhua-admin`</b></font>模块配置AnalysisService
...
在<font color=red><b>`tanhua-admin`</b></font>模块中创建LogMapper并配置查询方法
...
修改<font color=red><b>`tanhua-app-server`</b></font>模块中发布动态的方法，当动态发布完成，发送一条MQ消息
...
在<font color=red><b>`tanhua-admin`</b></font>模块中配置内容审核的监听器
...
修改<font color=red><b>`tanhua-dubbo-mongo`</b></font>模块中的`MovementApi与实现类`，添加修改状态的方法
